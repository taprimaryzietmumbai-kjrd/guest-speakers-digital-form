...
document.getElementById("speakerForm").addEventListener("submit",async e=>{
 ...
 const data={workshopTitle:workshopTitle,targetGroup:targetGroup,name:document.getElementById("name").value.trim(),rank:"",organization:org==="Others"?(document.getElementById("otherOrg").value.trim()||"Others"):org,whatsapp:document.getElementById("whatsapp").value.trim(),bank:document.getElementById("bank").value.trim(),ifsc:document.getElementById("ifsc").value.trim().toUpperCase(),upi:document.getElementById("upi").value.trim(),pan:document.getElementById("pan").value.trim().toUpperCase(),presentationDate:document.getElementById("presentationDate").value,fromDate:from,toDate:to,role:document.getElementById("role").value,signatureData:hasInk?canvas.toDataURL("image/png"):""};
 const btn=document.getElementById("saveBtn");btn.disabled=true;btn.textContent="Saving…";
 try{const r=await fetch(APPS_SCRIPT_URL,{method:"POST",body:JSON.stringify(data)});const j=await r.json();if(!j.ok)throw new Error(j.error||"Unable to save");
 if(j.pdfUrl){const s=document.getElementById("status");s.className="ok";s.innerHTML='✅ Speaker record saved successfully.<br>Record ID: '+j.recordId+'<br><a href="'+j.pdfUrl+'" target="_blank" rel="noopener" class="pdfLink">📄 View / Download Speaker PDF</a>';}else{status("✅ Speaker record saved successfully. Record ID: "+j.recordId,true);}
 }
